    ...
    )
{
    for (int i = 0; i < num_meshes; i++)
    {
        ...
        if (occlude_mesh(ray_t, &new_origin, &new_direction, mesh_min_bounds + mesh_offset, mesh_max_bounds + mesh_offset, mesh_tri_counts + mesh_offset, mesh_left_firsts + mesh_offset, mesh_triangles + triangle_offset))
        {
            return true;
        }
    }
